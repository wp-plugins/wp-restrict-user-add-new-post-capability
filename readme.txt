=== Remove ADD NEW POST Capability (from non-admin users) ===

Contributors: RSPublishing
Tags: add new, restrict posting, non-admin, user, user roles, prevent, limit, post limit, disable, remove, post, hide
Donate link: https://www.paypal.com/cgi-bin/webscr?cmd=_s-xclick&hosted_button_id=NMCXDH29TXS8U
Requires at least: 3.1
Tested up to: 4.2.2
Stable tag: 1.2
License : GPLv2 or later
License URI: http://www.gnu.org/licenses/gpl-2.0.html

This plugin specifically disables all non-admin users (with posting capability), from creating and adding new POSTS from the user dashboard.

== Description ==

This plugin specifically disables all non-admin users (with posting capability), from creating and adding new POSTS from the user dashboard. However, all standard roles 
assigned to the respective user levels (mainly EDITOR and AUTHOR), will remain the same.

The 'add new' submenu item will also be hidden from these users.

= About = 

The Remove ADD NEW POST Capability plugin is maintained by [YOOPlugins.com](http://yooplugins.com/)

== Installation ==

1. Download the napr.zip file
2. Upload and extract the contents of napr.zip to your wp-content/plugins/folder
3. Activate the plugin
4. Done! 

== Frequently Asked Questions ==
None

== Feedback, Help, Bugs and Suggestions ==

Just post an email to: rcstoltz@gmail.com with email subject: Remove 'ADD NEW POST' or visit us at [YOOPlugins](http://yooplugins.com/)

== Upgrade Notice ==

= Version 1.2 = 

== Screenshots ==

none

== Changelog ==

= Version 1.0 =

First Release Version

= 1.0.1 =
* tested compatibility with v3.7.1
* added blank index file to trunk (security)
* truncated initial plugin name
* included donation link
* corrected author name
* included temp banner
* updated keywords
* updated readme

= 1.0.2 =
*tested compatibility with v4.0
* general housekeeping

= 1.0.3 =
* changed url and support links
* general housekeeping

= 1.1 =
* tested compatibility with core 4.1.2
* updated version number
* changed support email
* general housekeeping
* renamed plugin name
* added rating link
* added icons

= 1.2 =
* tested compatibility with core 4.2.2
* added banner and icon assets
* updated version number
* general housekeeping
